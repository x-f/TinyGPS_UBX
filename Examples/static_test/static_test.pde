...
prog_char str5[] PROGMEM = "$PUBX,00,192539.00,5656.71213,N,02408.43532,E,24.054,G3,21,8.5,1.527,168.17,0.000,,1.46,2.39,1.66,5,0,0*7E";
prog_char str6[] PROGMEM = "$PUBX,00,192540.00,5656.71231,N,02408.43512,E,24.045,G3,21,8.5,1.125,168.17,0.000,,1.46,2.38,1.66,5,0,0*75";
prog_char str7[] PROGMEM = "$PUBX,00,171106.00,5656.70426,N,02408.44458,E,10.632,G3,39,45,0.468,189.36,0.586,,2.06,3.16,2.02,5,0,0*5C";
prog_char str8[] PROGMEM = "$PUBX,00,171842.00,5656.71193,N,02408.43523,E,42.845,G3,27,22,0.473,189.36,-0.173,,2.07,2.24,1.62,5,0,0*7F";

prog_char *teststrs[8] = {str1, str2, str3, str4, str5, str6, str7, str8};
...
void setup()
{
  ...
  Serial.println("by Mikal Hart (and x-f)");
  Serial.println();
...
void gpsdump(TinyGPS &gps)
{
  ...
  gps.get_time(&time, &age);
  Serial.print("Time(hhmmsscc): "); Serial.print(time);
  Serial.print(" Fix age: "); Serial.print(age); Serial.println("ms.");

  gps.crack_time(&hour, &minute, &second, &age);
  Serial.print("Time: "); Serial.print(static_cast<int>(hour)); Serial.print(":"); Serial.print(static_cast<int>(minute)); Serial.print(":"); Serial.print(static_cast<int>(second));
  Serial.print("  Fix age: ");  Serial.print(age); Serial.println("ms.");
  
  Serial.print("Alt(cm): "); Serial.print(gps.altitude()); Serial.print(" Course(10^-2 deg): "); Serial.print(gps.course()); 
  ///Serial.print(" Speed(10^-2 knots): "); Serial.println(gps.speed());
  Serial.print(" Alt(float): "); printFloat(gps.f_altitude(), 2); Serial.print(" Course(float): "); printFloat(gps.f_course(), 2); Serial.println();
  //Serial.print("Speed (knots): "); printFloat(gps.f_speed_knots(), 2); 
  Serial.print("Speed (mph): ");  printFloat(gps.f_speed_mph(), 2);
  Serial.print(" (kmph): "); printFloat(gps.f_speed_kmph(), 2); Serial.println();

  Serial.print("Vspeed (cm/s): "); Serial.println(gps.vspeed());
